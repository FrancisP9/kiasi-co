...
export function CinematicStudio() {
  return (
    <section className="bg-background text-foreground py-32 px-6 relative z-10 border-t border-foreground/5">
      <div className="container mx-auto max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          <div>
            <h2 className="text-xs font-light tracking-[0.3em] uppercase text-foreground/40 mb-8">
              Le Studio
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {team.map((member, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="aspect-[3/4] bg-foreground/5 mb-6 relative overflow-hidden rounded-sm">
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                {/* Placeholder for team photo */}
                <div className="absolute inset-0 flex items-center justify-center text-foreground/10 text-xl font-serif italic">
                  {member.name}
                </div>
              </div>
              ...
              <p className="text-sm text-foreground/60 leading-relaxed border-t border-foreground/10 pt-4">
                {member.bio}
              </p>
            </motion.div>
          ))}
